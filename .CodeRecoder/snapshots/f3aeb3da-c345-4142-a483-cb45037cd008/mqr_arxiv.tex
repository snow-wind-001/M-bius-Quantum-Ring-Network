...
\begin{itemize}
    \item \textbf{Unitary evolution}: The state update $\Psi_{next} = U\Psi_{curr}$ mimics Schrödinger dynamics.
    \item \textbf{Measurement}: Local projective readout is analogous to quantum measurement projecting onto a subspace.
    \item \textbf{Phase interference}: Parameter updates through phase rotation create ``interference patterns'' in the doubly stochastic weights.
\end{itemize}

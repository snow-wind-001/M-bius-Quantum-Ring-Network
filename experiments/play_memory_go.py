"""Continue the v2 multi-ring / raw-memory student online against the local teacher."""
...
from go_memory_research import build, play_games, session_for
...
def main():
    ...
    parser.add_argument("--output", default="analysis/results/go_memory_continuation.json")
    ...
    if method == "frozen":
        parser.error("the frozen control has no online trainable parameters")
    ...
    destination = Path(options.save); destination.parent.mkdir(parents=True, exist_ok=True)

"""Larger complete-game study of shared-policy values and margin constraints.

No move cap creates a result. A compute interruption saves the unfinished
board/session and resumes exactly that game. See analysis/go11_plan.md.
"""
...
METHODS = ("mixed_ogd3", "selfplay_ogd3", "selfplay_cone3", "selfplay_cone6", "selfplay_free6", "frozen6")
...
def build(args, method, seed):
    ...
    rates = (1.0, 0.15, 0.03) if method.endswith("3") else (1.0, 0.5, 0.15, 0.07, 0.03, 0.01)
    core = GoResearchCore(
        encoder.output_dim, 16, 16, board_size=args.size, fixed_colors=True,
        conditional_scale=0.3, transition_mode="orthogonal",
    ...
    shared = dict(board_size=args.size, core=core, ring_dim=16, latent_dim=16,
                  channels=12, query_dim=8, task_lr=0.02, max_update_norm=0.08,
...
def make_session(args, agent, encoder, method):
    protected = method not in ("frozen6", "selfplay_free6")
    cone = "cone" in method
    memory_type = TaskMarginMemory if cone else PolicyBehaviorMemory
    memory = memory_type(4, strata=4, reliable_only=True, margin_fraction=0.5) if protected else None
    return SharedPolicyGoSession(agent, encoder, update_every=args.window, credit_horizon=args.window,
        shared_policy=method != "mixed_ogd3", project_with_memory=protected and not cone,
        behavior_memory=memory, refresh_every=1, max_anchor_kl=0.05 if protected and not cone else None,
        replay_mode="none" if method == "frozen6" else "outcome", outcome_weight=0.5, episode_capacity=4096)
...
def prepare(args):
    ...
    agent, _ = build(args, "selfplay_cone3", args.seed)
    base = agent.spatial_skip_heads
    base.requires_grad_(True)
    nn.init.constant_(base.pass_decision.bias, -math.log(args.size ** 2))
    # Teacher-versus-teacher returns are not shared learner-policy values.
    nn.init.zeros_(base.value.weight)
    nn.init.zeros_(base.value.bias)
    optimizer = torch.optim.Adam(base.parameters(), lr=0.006)
    generator = torch.Generator().manual_seed(args.seed + 32000)
    weights = GoLossWeights(value=0, illegal_mass=0.2)
    ...
    data = {"version": 1, "source": source_hashes(), "size": args.size, "seed": args.seed,
            "pretrained_value_semantics": "zero; teacher returns excluded from training",
...
def stages(args, method):
    ...
    result.append(("search_eval", args.eval_games, False, args.eval_simulations))
    if method in ("selfplay_cone6", "frozen6"):
        result.append(("budget16_eval", args.eval_games, False, 16))
    return result
...
def compact_update(result):
    if result is None:
        return None
    return {key: result[key] for key in ("did_update", "update_norm", "parameter_version", "anchor_drift", "constraint_projection",
...
def finish_record(game, board, terminal):
    ...
    return {key: game[key] for key in ("phase", "index", "opening", "student_color", "decisions", "updates",
        ...
        "replayed_context": terminal.get("replayed_context", 0),
        "value_semantics": terminal.get("value_semantics", "evaluation_only"),
    }


def search_decision(args, agent, encoder, board, result, *, phase, index, simulations, training):
    """Shared stochastic training policy; deterministic frozen evaluation."""
    scores = result["policy_logits"]
    rng = None
    if training:
        rng = np.random.default_rng([args.seed, ("a", "b", "return_a").index(phase), index, len(board.move_history)])
        legal = board.legal_moves()
        probabilities = scores[0, legal].softmax(0).double().numpy()
        noisy = 0.85 * probabilities + 0.15 * rng.dirichlet(np.full(len(legal), 0.3))
        scores = torch.full_like(scores, math.log(1e-30))
        scores[0, legal] = torch.as_tensor(np.log(noisy.clip(1e-30)), dtype=scores.dtype)
    search = policy_value_search(agent, encoder, board, result["state"], scores,
                                 simulations=simulations, max_depth=args.search_depth)
    if training and len(board.move_history) < 80:
        choices = list(search["visits"])
        visits = np.array([search["visits"][a] for a in choices], dtype=float)
        search["action"] = int(rng.choice(choices, p=visits / visits.sum()))
    return search
...
def run(args):
    ...
    result_path = Path(args.output or ROOT / "analysis/results" / f"go11_{method}_seed{args.seed}.json")
    ...
    if work_path.exists():
        ...
        if method not in ("frozen6", "selfplay_free6"):
        ...
        work = {"version": 1, "config": config_record(args), "source": source_hashes(),
                ...
                "trainable_parameters": 0 if method == "frozen6" else sum(p.numel() for p in agent.parameters() if p.requires_grad),
                "value_target_contract": "mixed_behavior" if method == "mixed_ogd3" else "shared_policy",
                "probe_value_scope": "teacher behavior returns are diagnostic only; not shared-policy ground truth"}
    ...
    while work["stage"] < len(schedule):
        ...
        learn = training and method != "frozen6"
        while not board.game_over:
            started = time.perf_counter()
            result = session.observe(board, learn=learn)
            student = (training and method != "mixed_ogd3") or board.to_play == game["student_color"]
            search = None
            if student and simulations:
                search = search_decision(args, agent, encoder, board, result, phase=phase,
                                         index=game["index"], simulations=simulations, training=training)
...
def parse_args():
    ...
    parser.add_argument("--method", choices=METHODS, default="selfplay_cone6")
    parser.add_argument("--size", type=int, default=10)
    parser.add_argument("--directory", default=str(ROOT / "checkpoints/go11_v1"))
    parser.add_argument("--output")
    parser.add_argument("--warmup-games", type=int, default=24)
    parser.add_argument("--epochs", type=int, default=24)
    parser.add_argument("--train-games", type=int, default=8, help="complete games in each of A/B/A")
    parser.add_argument("--eval-games", type=int, default=48)
    parser.add_argument("--window", type=int, default=16)
    parser.add_argument("--simulations", type=int, default=32, help="training simulations for all methods")
    ...
    if (args.size != 10 or args.warmup_games < 1 or args.epochs < 1 or args.train_games < 1
            ...
            or args.simulations < 1 or args.eval_simulations < 1 or args.search_depth < 1 or args.max_plies < 0):
    ...
    if args.confirmation and (args.seed not in SEEDS or args.warmup_games != 24 or args.epochs != 24
                             or args.train_games != 8 or args.eval_games != 48 or args.window != 16
                             or args.simulations != 32 or args.eval_simulations != 64 or args.search_depth != 16):

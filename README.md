...
> 支持批量历史、截断信用、当前参数刷新及在线恢复。
> [修改方案](analysis/constraint_transport_plan.md)；[实现、数学合同与重新实验结果](analysis/constraint_transport_report.md)。
> 此接口要求线性环状态和外生输入条件化，不能用于任意隐状态控制器或非负模平方传播。

> **多环阶段证据（2026-09-11）**：已实现几何空间分支、固定黑白记忆坐标、多时间尺度正交环、
> 输入条件化旋转、按位置查询和有界原始观测记忆。在线系统支持独立信用跨度、训练锚点 OGD 刷新、
> 有限步策略 KL 约束以及包含两类记忆的恢复。使用合法历史配对、学生在线对局和独立种子做对照。
> 详见 [多环与外部记忆研究报告、实测结果及复现命令](analysis/go_memory_research_report.md)。
> 五种子历史探针中，正交环短／完整信用准确率为 50.78% / 90%，条件正交环为 100%；
> 围棋冻结基线 3/40 胜，正交环在线更新 16/40，直接微调骨干 17/40。整体适应有收益，
> 环结构的独特优势和外部记忆的额外收益尚未建立。139 项回归测试及 1080 份棋谱审计通过。

> **上一轮证据（2026-09-09）**：按新的研究要求恢复围棋在线探索，保留正交、环形结构与在线更新。
...
[原技术根因分析与解决队列](analysis/current_technical_root_causes.md) 保留了 40 个旧版 checkpoint 的只读诊断：空间版本的空棋盘位置别名、记忆视角逐手翻转、较弱的环转移梯度及 OGD 锚点漂移。新结构作为可选接口接入，历史默认配置继续用于复现旧证据。
